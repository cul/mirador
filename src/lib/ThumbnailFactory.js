...
function getBestThumbnail(resource, iiifOpts, miradorConfig = {}) {
  const canvasResourceTypes = (miradorConfig.canvas && miradorConfig.canvas.resourceTypes) || {};
  const imageServiceProfiles = (miradorConfig.image && miradorConfig.image.serviceProfiles) || {};
  return new ThumbnailFactory(
    iiifOpts,
    {
      getMiradorCanvas: (r) => new MiradorCanvas(r, canvasResourceTypes, imageServiceProfiles),
      getMiradorManifest: (r) => new MiradorManifest(r, canvasResourceTypes),
    },
  ).get(resource);
}
