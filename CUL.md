...
1. When you are done making changes on your feature branch, create a new branch with the changes that follows this naming pattern:`cul-v${mirador-version}-{cul-release-version}-rebase`.  For example: `cul-v4.0.0-alpha.16-4-rebase` (Derived from these four pieces: `cul-` `v4.0.0-alpha.16` `-4-` `rebase`).
...
3. Run `npm run build` to verify that this package builds successfully.

4. Add a new commit with message: `TEMP: bump build version`

5. Follow the normal NPM package publishing steps (https://docs.npmjs.com/creating-and-publishing-scoped-public-packages).  Here a couple of reminders:
